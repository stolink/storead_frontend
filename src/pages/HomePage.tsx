...
const WorkCard = ({ work }: { work: Work }) => {
  ...
  const avgRating =
    work.ratingCount > 0 ? work.ratingSum / work.ratingCount : 0;
    ...
    >
      ...
      <div className="relative aspect-[3/4] overflow-hidden rounded-t-lg">
        ...
        <span className="absolute top-2 left-2 px-2 py-1 text-xs font-medium bg-black/60 text-white rounded">
          {GENRE_FILTERS.find((g) => g.value === work.genre)?.label ||
            work.genre}
        </span>
      </div>
...
export const HomePage = () => {
  ...
  // 백엔드 응답: { data: { pagination, works: [...] } }
  const works = isSearching
    ? searchData?.data?.works
    : discoveryData?.data?.works;

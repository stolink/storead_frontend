...
export const useCategoryWorks = (
  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 0 }) => {
      ...
      if (params?.accessType) queryParams.accessType = params.accessType;
      if (params?.keyword) queryParams.keyword = params.keyword;

      const { data } = await api.get("/discovery/works", {
        params: queryParams,
      });
...
/**
 * 작가의 다른 작품 조회 (공개용)
 * GET /api/discovery/works?authorId={authorId}
 */
export const usePublicAuthorWorks = (authorId: string | undefined) => {
  return useQuery<Work[]>({
    queryKey: ["discovery", "author", authorId],
    queryFn: async () => {
      const { data } = await api.get("/discovery/works", {
...
export const useRankings = (
  ...
  return useQuery<PaginatedResponse<Work>>({
    queryKey: ["rankings", period, genre, accessType],
    queryFn: async () => {
      const { data } = await api.get("/discovery/rankings", {
        params: { period, genre, accessType, size: 100 },
      });
      const responseData = data.data;
      return {
        data: responseData?.works || [],

...
import {
  useChapterPurchase,
  useChapterPurchaseCheck,
} from "@/hooks/useChapterPurchase";
...
export function PurchaseConfirmModal({
  ...
  // 구매 가능 여부 확인
  const { data: purchaseCheck, isLoading: checkLoading } =
    useChapterPurchaseCheck(chapter.id);
            ...
            {step === "confirm" && (
              <>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    제{chapter.chapterNumber}화 "{chapter.title}"을(를)
                    구매하시겠습니까?
                  </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4 py-4">
                  ...
                  <div className="bg-mocha-50 dark:bg-mocha-900/20 rounded-lg p-4 space-y-3">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span
                        className={`font-semibold ${!canAfford ? "text-red-500" : ""}`}
                      >
                        {currentBalance} C
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span
                        className={`font-semibold ${afterBalance < 0 ? "text-red-500" : "text-sage-600"}`}
                      >
                        {afterBalance >= 0 ? afterBalance : "잔액 부족"}{" "}
                        {afterBalance >= 0 && "C"}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start gap-2 p-3 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-lg text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-red-600 dark:text-red-300">
                          {Math.abs(afterBalance)} C가 더 필요합니다. 충전 후
                          구매해 주세요.
                        </p>
                      </div>
                    </div>
                ...
                <DialogFooter className="gap-2 sm:gap-0">
                  ...
                  {canAfford ? (
                    ...
                    <Button onClick={handleGoToCharge}>충전하러 가기</Button>
                  )}
                </DialogFooter>
              </>
            )}
            ...
            {step === "insufficient" && (
              <>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    이 챕터를 구매하려면 {Math.abs(afterBalance)} C가 더
                    필요합니다.
                  </DialogDescription>
                </DialogHeader>
                <DialogFooter className="gap-2 sm:gap-0">

...
const GENRE_GROUPS: Record<string, { label: string, tabs: { label: string, value: string }[] }> = {
    ...
    'FANTASY': {
        label: '판타지',
        tabs: [
            { label: '전체', value: 'FANTASY,HEROIC_FANTASY,DARK_FANTASY,URBAN_FANTASY,HIGH_FANTASY,ISEKAI,MODERN_FANTASY,TRADITIONAL_FANTASY,ROMANCE_FANTASY' },
...
export const CategoryPage = () => {
    ...
    // URL 파라미터 상태 동기화
    const initialSubGenre = searchParams.get('sub') || (GENRE_GROUPS[genreId!]?.tabs[0].value || genreId);

    // 상태 관리
    const [selectedGenreValue, setSelectedGenreValue] = useState<string | undefined>(initialSubGenre);
    const [statusFilter, setStatusFilter] = useState<string>(searchParams.get('status') || ''); // '' or 'ONGOING', 'COMPLETED'
    ...
    const {
        ...
        status: statusFilter,
        sort: sortBy === 'latest' ? 'createdAt' : sortBy === 'popular' ? 'likeCount' : 'rating',
        order: 'desc',
        limit: 20
    });
    ...
    return (
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-900 pb-20">
            {/* 헤더 & 탭 */}
            <div className="bg-white dark:bg-zinc-800 border-b border-zinc-200 dark:border-zinc-700 sticky top-0 z-20">
                    ...
                    <div className="flex justify-between items-end mb-6">
                        <div>
                            ...
                            <p className="text-zinc-500 dark:text-zinc-400 font-serif italic">
                                Discover the best {title} novels currated for you.
                            </p>
                        </div>
...
{/* Follow Genre 버튼 - 백엔드 API 구현 후 활성화
                        ...
                        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-2">
                            {currentGroup.tabs.map((tab) => (
                                <button
                                    ...
                                    className={`px-5 py-2 rounded-full text-sm font-bold tracking-wide transition-all duration-300 ${selectedGenreValue === tab.value
                                        ? 'bg-zinc-900 text-white shadow-lg scale-105 dark:bg-zinc-100 dark:text-zinc-900'
                                        : 'bg-zinc-100 text-zinc-500 hover:bg-zinc-200 hover:text-zinc-900 dark:bg-zinc-700 dark:text-zinc-400'
                                        }`}
                                >
                                    {tab.label}
                                </button>
                ...
                {/* 필터 바 */}
                <div className="container mx-auto px-6 py-3 flex items-center justify-between border-t border-zinc-100 dark:border-zinc-700 bg-zinc-50/50 dark:bg-zinc-800/50 backdrop-blur-sm">
                    <div className="flex gap-2 items-center">
                        <span className="text-xs font-bold uppercase tracking-wider text-zinc-400 mr-2">Filter</span>
                        <FilterBadge
                            label="ALL"
            ...
            <div className="container mx-auto px-6 py-8">
                {isLoading ? (
                    ...
                    <>
                        {data?.pages.map((page, i) => (
                            ...
                            >
                                {page.data.map((work: Work) => (
                                    viewMode === 'grid' ? (
                                        <BookCard key={work.id} work={work} showQuickActions={true} />
                                    ) : (
                        ...
                        {/* 무한 스크롤 트리거 */}
                        <div ref={containerRef} className="h-10 flex justify-center items-center">
                            {isFetchingNextPage && <Loader2 className="w-6 h-6 animate-spin text-zinc-400" />}
                        </div>
                    </>
                )}
            </div>
    ...
    return (
        <button
            ...
            className={`px-3 py-1 text-xs rounded-full border transition-colors ${isActive
                ...
                : 'bg-white text-zinc-600 border-zinc-200 hover:border-zinc-300 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-400'
                }`}
        >
            {label}
        </button>
...
function ListViewItem({ work }: { work: Work }) {
    ...
    return (
        <div
            ...
            className="flex gap-4 p-4 bg-white dark:bg-zinc-800 rounded-lg border border-zinc-100 dark:border-zinc-700 hover:shadow-md transition-shadow cursor-pointer"
        >
            {/* 썸네일 */}
            ...
            <div className="flex-1 py-1">
                <div className="flex justify-between items-start">
                    <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 mb-1">{work.title}</h3>
                ...
                <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-3">{work.author?.nickname || work.authorNickname}</p>
                <p className="text-sm text-zinc-500 line-clamp-2 mb-3">{work.synopsis}</p>

                <div className="flex gap-4 text-xs text-zinc-400">
                    <span className="flex items-center gap-1">
                        <Star className="w-3 h-3 fill-yellow-400 text-yellow-400" />
                        {work.ratingSum && work.ratingCount ? (work.ratingSum / work.ratingCount).toFixed(1) : '0.0'}
                    </span>
                    <span className="flex items-center gap-1">
                        <MessageSquare className="w-3 h-3" />
                        댓글 0 {/* 댓글 수는 Work 엔티티에 없어서 0으로 표시하거나 API 수정 필요 */}
                    </span>
                </div>
            </div>
        </div>

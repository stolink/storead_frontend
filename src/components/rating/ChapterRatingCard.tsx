...
export const ChapterRatingCard = ({
  ...
  const [previousRating, setPreviousRating] = useState(currentRating);

  // props로 들어온 currentRating이 변경되면 상태 업데이트 (Render phase update)
  ...
  return (
    ...
    >
      ...
      >
        {showThankYou ? "참여해주셔서 감사합니다! ✨" : "이 회차는 어떠셨나요?"}
      </p>
      ...
      {ratingCount > 0 && (
        <p className={cn("text-center text-xs", cardStyle.subText)}>
          평균 {(avgRating / 2).toFixed(1)} ({ratingCount.toLocaleString()}명
          참여)
        </p>
      )}
    </div>
